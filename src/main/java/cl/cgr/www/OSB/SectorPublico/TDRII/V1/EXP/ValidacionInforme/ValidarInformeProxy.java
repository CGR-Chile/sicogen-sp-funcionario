...
public class ValidarInformeProxy implements ValidarInforme {
  ...
  private void _initValidarInformeProxy() {
    try {
      validarInforme = (new cl.cgr.www.OSB.SectorPublico.TDRII.V1.EXP.ValidacionInforme.ValidarInformeSOAPQSServiceLocator()).getValidarInformeSOAPQSPort();
  ...
  private void _initValidarInformeProxy(int idWS) {
    try {
      validarInforme = (new cl.cgr.www.OSB.SectorPublico.TDRII.V1.EXP.ValidacionInforme.ValidarInformeSOAPQSServiceLocator(idWS)).getValidarInformeSOAPQSPort();
      if (validarInforme != null) {
        if (_endpoint != null)
          ((javax.xml.rpc.Stub)validarInforme)._setProperty("javax.xml.rpc.service.endpoint.address", _endpoint);
        else
